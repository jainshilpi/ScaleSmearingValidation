...
vector<float>  elePt_;
vector<float>  eleEnPreSS_;
vector<float>  elePtPreSS_;
vector<float>  elePtErrPreSS_;
vector<float>  elePtError_;

vector<float>  eleEnErrPreSS_;
vector<float>  eleCalibEnErr_;
...
vector<float>  eleScale_et_up_;
vector<float>  eleScale_et_dn_;
...
void ggNtuplizer::branchesElectrons(TTree* tree) {
  ...
  tree->Branch("eleEnPreSS",                   &eleEnPreSS_);
  tree->Branch("elePtPreSS",                   &elePtPreSS_);
  tree->Branch("elePtErrPreSS",                   &elePtErrPreSS_);
  tree->Branch("elePtError",              &elePtError_);

  tree->Branch("eleEnErrPreSS",                   &eleEnErrPreSS_);
  tree->Branch("eleCalibEnErr",                   &eleCalibEnErr_);
  ...
  tree->Branch("eleScale_et_up",            &eleScale_et_up_);
  tree->Branch("eleScale_et_dn",            &eleScale_et_dn_);

  tree->Branch("eleSeedSCGain",                    &eleSeedSCGain_);
...
void ggNtuplizer::fillElectrons(const edm::Event &e, const edm::EventSetup &es, math::XYZPoint &pv) {
  ...
  elePtErrPreSS_                      .clear();

  eleEnErrPreSS_                      .clear();
  eleCalibEnErr_                      .clear();
  ...
  eleScale_et_up_           .clear();
  eleScale_et_dn_           .clear();

  eleSeedSCGain_            .clear();
  ...
  if(debug) std::cout<<"gettign vtx handle"<<std::endl;
  ...
  for (edm::View<pat::Electron>::const_iterator iEle = electronHandle->begin(); iEle != electronHandle->end(); ++iEle) {
    ...
    eleEnPreSS_              .push_back(iEle->userFloat("ecalEnergyPreCorr"));
    eleCalibEn_         .push_back(iEle->userFloat("ecalEnergyPostCorr"));

    eleEnErrPreSS_              .push_back(iEle->userFloat("ecalEnergyErrPreCorr"));
    eleCalibEnErr_         .push_back(iEle->userFloat("ecalEnergyErrPostCorr"));
    ...
    elePtPreSS_         .push_back(iEle->userFloat("ecalTrkEnergyPreCorr")*iEle->pt()/iEle->p());
    
    elePtErrPreSS_         .push_back(iEle->userFloat("ecalTrkEnergyErrPreCorr")*iEle->pt()/iEle->p());
    ...
    //https://cmssdt.cern.ch/lxr/source/PhysicsTools/NanoAOD/plugins/EGMSeedGainProducer.cc?v=CMSSW_11_1_X_2020-03-02-2300#0082
    const DetId seedDetId = iEle->superCluster()->seed()->seed();
    auto recHits = iEle->isEB() ? recHitsEB.product() : recHitsEE.product();
    ...
    eleSeedSCGain_ .push_back(gainSeedSC);
    ...
    eleScale_et_up_.push_back(iEle->userFloat("energyScaleEtUp"));
    eleScale_et_dn_.push_back(iEle->userFloat("energyScaleEtDown"));
