...
void ggNtuplizer::fillGlobalEvent(const edm::Event& e, const edm::EventSetup& es) {
  ...
  if(debug) std::cout<<"gettign rho handle"<<std::endl;
  ...
  if (vtxHandle.isValid()) {
    ...
    if(debug) std::cout<<"iterating vtx handle"<<std::endl;
  ...
  for (size_t i = 0; i < trgNames.size(); ++i) {
    ...
    if(debug) std::cout<<"Took all the triggers, done with GE"<<std::endl;
